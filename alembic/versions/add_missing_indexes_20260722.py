...
from migration_helpers import create_index_if_not_exists
...
def upgrade() -> None:
    ...
    if _has_column("message_logs", "tenant_id"):
        create_index_if_not_exists("ix_message_logs_tenant_created", "message_logs", ["tenant_id", "created_at"])
    create_index_if_not_exists("ix_message_logs_recipient_source", "message_logs", ["recipient", "source"])

    # broadcasts
    create_index_if_not_exists("ix_broadcasts_account_status", "broadcasts", ["account_id", "status"])

    # accounts
    if _has_column("accounts", "tenant_id"):
        create_index_if_not_exists("ix_accounts_tenant_status", "accounts", ["tenant_id", "status"])

    # leads
    if _has_column("leads", "tenant_id"):
        create_index_if_not_exists("ix_leads_tenant_active", "leads", ["tenant_id", "is_active"])

    # auto_reply_logs
    create_index_if_not_exists("ix_auto_reply_logs_account_status_created",
                    "auto_reply_logs", ["account_id", "status", "created_at"])

    # reply_macro_logs
    create_index_if_not_exists("ix_reply_macro_logs_account_status_created",
                    "reply_macro_logs", ["account_id", "status", "created_at"])

    # usage_records
    if _has_column("usage_records", "tenant_id"):
        create_index_if_not_exists("ix_usage_records_tenant_action_recorded",
                        "usage_records", ["tenant_id", "action", "recorded_at"])

    # ai_chat_logs
    if _has_column("ai_chat_logs", "tenant_id"):
        create_index_if_not_exists("ix_ai_chat_logs_tenant_session",
                        "ai_chat_logs", ["tenant_id", "session_id"])

    # campaigns
    if _has_column("campaigns", "tenant_id"):
        create_index_if_not_exists("ix_campaigns_tenant_status", "campaigns", ["tenant_id", "status"])

    # message_templates
    if _has_column("message_templates", "tenant_id"):
        create_index_if_not_exists("ix_message_templates_tenant_category",
                        "message_templates", ["tenant_id", "category"])

    # team_members
    if _has_column("team_members", "tenant_id"):
        create_index_if_not_exists("ix_team_members_tenant_role", "team_members", ["tenant_id", "role"])

    # follow_up_rules
    create_index_if_not_exists("ix_follow_up_rules_tenant_account_active",
                    "follow_up_rules", ["tenant_id", "account_id", "is_active"])

    # join_queue_items
    create_index_if_not_exists("ix_join_queue_items_account_status",
                    "join_queue_items", ["account_id", "status"])

    # BroadcastScheduleEntry
    if _has_column("broadcast_schedule_entries", "tenant_id"):
        create_index_if_not_exists("ix_broadcast_schedule_entries_tenant_status",
