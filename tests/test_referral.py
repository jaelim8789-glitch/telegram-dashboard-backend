...
@pytest.mark.asyncio
async def test_generate_referral_code(unauthenticated_client: AsyncClient):
...
@pytest.mark.asyncio
async def test_referral_dashboard_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/dashboard should require auth."""
    res = await unauthenticated_client.get("/api/referrals/dashboard")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_admin_pending_commissions_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/admin/pending should require admin auth."""
    res = await unauthenticated_client.get("/api/referrals/admin/pending")
    assert res.status_code == 401
...
async def test_referral_code_generation_and_uniqueness(db_session):
    ...
    async with app.database.async_session_maker() as db:
        owner = Tenant(phone="+821099999991", plan="free")
        db.add(owner)
        await db.flush()

        code1 = ReferralCode(code="TEST1234", owner_id=owner.id)
        db.add(code1)
        await db.flush()

        code2 = ReferralCode(code="TEST5678", owner_id=owner.id)
        db.add(code2)
        await db.flush()

        result = await db.execute(
            __import__("sqlalchemy").select(ReferralCode).where(ReferralCode.code == "TEST1234")
        )
        found = result.scalar_one_or_none()
        assert found is not None
        assert found.code == "TEST1234"
...
@pytest.mark.asyncio
async def test_referral_link_endpoint(unauthenticated_client: AsyncClient):
    """GET /api/referrals/my-link should return a Telegram deep link."""
    res = await unauthenticated_client.get("/api/referrals/my-link")
    assert res.status_code == 401
...
@pytest.mark.asyncio
async def test_leaderboard_endpoint_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/leaderboard should be publicly accessible."""
    res = await unauthenticated_client.get("/api/referrals/leaderboard")
    assert res.status_code == 200


@pytest.mark.asyncio
async def test_admin_payouts_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/admin/payouts should require admin."""
    res = await unauthenticated_client.get("/api/referrals/admin/payouts")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_admin_process_payouts_no_auth(unauthenticated_client: AsyncClient):
    """POST /api/referrals/admin/process-payouts should require admin."""
    res = await unauthenticated_client.post("/api/referrals/admin/process-payouts")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_admin_pending_payouts_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/admin/payouts/pending should require admin."""
    res = await unauthenticated_client.get("/api/referrals/admin/payouts/pending")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_admin_approve_payout_no_auth(unauthenticated_client: AsyncClient):
    """POST /api/referrals/admin/payouts/{id}/approve should require admin."""
    res = await unauthenticated_client.post("/api/referrals/admin/payouts/fake-id/approve")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_referral_stats_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/stats should require auth."""
    res = await unauthenticated_client.get("/api/referrals/stats")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_set_chat_id_no_auth(unauthenticated_client: AsyncClient):
    """POST /api/referrals/set-chat-id should require auth."""
    res = await unauthenticated_client.post("/api/referrals/set-chat-id", json={"chat_id": "12345"})
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_admin_cancel_commission_no_auth(unauthenticated_client: AsyncClient):
    """POST /api/referrals/admin/commissions/{id}/cancel should require admin."""
    res = await unauthenticated_client.post("/api/referrals/admin/commissions/fake-id/cancel")
    assert res.status_code == 401
...
@pytest.mark.asyncio
async def test_csv_download_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/stats/csv should require auth."""
    res = await unauthenticated_client.get("/api/referrals/stats/csv")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_admin_commissions_csv_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/admin/commissions/csv should require admin."""
    res = await unauthenticated_client.get("/api/referrals/admin/commissions/csv")
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_change_code_no_auth(unauthenticated_client: AsyncClient):
    """POST /api/referrals/change-code should require auth."""
    res = await unauthenticated_client.post("/api/referrals/change-code", json={"new_code": "NEWCODE"})
    assert res.status_code == 401


@pytest.mark.asyncio
async def test_qr_endpoint_no_auth(unauthenticated_client: AsyncClient):
    """GET /api/referrals/my-qr should require auth."""
    res = await unauthenticated_client.get("/api/referrals/my-qr")
    assert res.status_code == 401
...
@pytest.mark.asyncio
async def test_my_commissions_endpoint(unauthenticated_client: AsyncClient):
    """GET /api/referrals/my-commissions should return commission list for authenticated user."""
    res = await unauthenticated_client.get("/api/referrals/my-commissions")
    assert res.status_code == 401
...
@pytest.mark.asyncio
async def test_set_wallet_endpoint(unauthenticated_client: AsyncClient):
    """POST /api/referrals/set-wallet should store wallet address."""
    res = await unauthenticated_client.post("/api/referrals/set-wallet", json={"wallet_address": "0x1234567890abcdef"})
    assert res.status_code == 401
    ...
    from app.core.rate_limiter import reset_rate_limits
    reset_rate_limits()
    res = await unauthenticated_client.post("/api/referrals/set-wallet",
...
@pytest.mark.asyncio
async def test_admin_settings_endpoint(unauthenticated_client: AsyncClient, admin_token: str):
    """GET/PUT /api/referrals/admin/settings should work for admin only."""
    from app.core.rate_limiter import reset_rate_limits
    reset_rate_limits()

    # no auth
    res = await unauthenticated_client.get("/api/referrals/admin/settings")
    assert res.status_code == 401

    # admin auth
    res = await unauthenticated_client.get("/api/referrals/admin/settings", headers={"Authorization": f"Bearer {admin_token}"})
    assert res.status_code == 200
    data = res.json()
    assert "settings" in data

    # update settings
    res = await unauthenticated_client.put("/api/referrals/admin/settings",
...
@pytest.mark.asyncio
async def test_admin_codes_endpoint(unauthenticated_client: AsyncClient, admin_token: str):
    """GET /api/referrals/admin/codes should return code stats for admin only."""
    res = await unauthenticated_client.get("/api/referrals/admin/codes")
    assert res.status_code == 401

    res = await unauthenticated_client.get("/api/referrals/admin/codes", headers={"Authorization": f"Bearer {admin_token}"})
    assert res.status_code == 200
    data = res.json()
...
@pytest.mark.asyncio
async def test_referral_rate_limit(unauthenticated_client: AsyncClient):
    ...
    for _ in range(10):
        res = await unauthenticated_client.post("/api/referrals/generate", headers={"Authorization": f"Bearer {user_jwt}"})
        assert res.status_code == 200
