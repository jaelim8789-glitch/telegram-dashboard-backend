...
class PaymentRecord(Base):
    ...
    # TronGrid block_timestamp is milliseconds-since-epoch (13 digits) — exceeds int32 range.
    block_timestamp: Mapped[int] = mapped_column(BigInteger, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())


class StarsPaymentRecord(Base):
    """Idempotency + audit log for native Telegram Stars top-ups (real XTR
    payment via bot.send_invoice), separate from the internal stars_balance
    spend/earn ledger. telegram_payment_charge_id is unique — Telegram can
    redeliver the successful_payment update, this guards against double credit.
    """
