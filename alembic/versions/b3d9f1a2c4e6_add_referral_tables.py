...
def upgrade() -> None:
    ...
    if "tenants" in existing_tables:
        ...
        if not table_has_column("tenants", "telegram_chat_id"):
            add_column_if_not_exists("tenants", sa.Column("telegram_chat_id", sa.String(length=100), nullable=True))
        if not table_has_column("tenants", "wallet_address"):
            add_column_if_not_exists("tenants", sa.Column("wallet_address", sa.String(length=100), nullable=True))
