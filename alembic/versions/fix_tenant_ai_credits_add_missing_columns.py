...
def upgrade() -> None:
    ...
    if not _column_exists("tenants", "ai_credits_remaining"):
        op.add_column("tenants", sa.Column("ai_credits_remaining", sa.Integer, server_default="0"))
    if not _column_exists("tenants", "ai_credits_reset_tokens"):
        op.add_column("tenants", sa.Column("ai_credits_reset_tokens", sa.Integer, server_default="0"))
    if not _column_exists("tenants", "ai_last_refill_at"):
        op.add_column("tenants", sa.Column("ai_last_refill_at", sa.DateTime(timezone=True), nullable=True))

    # Wallet
    if not _column_exists("tenants", "wallet_address"):
        op.add_column("tenants", sa.Column("wallet_address", sa.String(100), nullable=True))

    # Distributor
    if not _column_exists("tenants", "is_distributor"):
        op.add_column("tenants", sa.Column("is_distributor", sa.Boolean, server_default="false"))

    # Check-in
    if not _column_exists("tenants", "checkin_streak"):
        op.add_column("tenants", sa.Column("checkin_streak", sa.Integer, server_default="0"))
    if not _column_exists("tenants", "last_checkin_at"):
        op.add_column("tenants", sa.Column("last_checkin_at", sa.DateTime(timezone=True), nullable=True))
    if not _column_exists("tenants", "trial_expiry_notified"):
        op.add_column("tenants", sa.Column("trial_expiry_notified", sa.Boolean, server_default="false"))

    # Stars balance
    if not _column_exists("tenants", "stars_balance"):
        op.add_column("tenants", sa.Column("stars_balance", sa.Integer, server_default="0"))
